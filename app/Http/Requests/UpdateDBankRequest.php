...
class UpdateDBankRequest extends FormRequest
{
    /**
    ...
    {
        $currentNoRekening = $this->route('noRekening');
        ...
        return [
            ...
            'no_rekening' => [
                ...
                'string',
                'max:50',
                Rule::unique('d_bank')->where(function ($query) use ($currentNoRekening) {
                    return $query->where('kode_divisi', $this->route('kodeDivisi'))
                                 ->where('no_rekening', '!=', $currentNoRekening);
                })
            ],
