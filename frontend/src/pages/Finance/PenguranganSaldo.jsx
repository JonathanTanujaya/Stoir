...
import { faMinus, faPlus, faEdit, faTrash, faEye, faSearch, faCalendarAlt, faMoneyBillWave } from '@fortawesome/free-solid-svg-icons';
...
const PenguranganSaldo = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (editingId) {
      setSaldoData(prev => prev.map(item => 
        item.id === editingId ? { 
          ...formData, 
          id: editingId, 
  ...
  const getJenisTransaksiColor = (jenis) => {
    switch (jenis) {
      case 'Penarikan Tunai': return 'bg-blue-100 text-blue-800';
      case 'Transfer Keluar': return 'bg-purple-100 text-purple-800';
      case 'Biaya Admin': return 'bg-gray-100 text-gray-800';
      case 'Pajak': return 'bg-orange-100 text-orange-800';
      case 'Investasi': return 'bg-green-100 text-green-800';
        ...
        <div className="bg-white rounded-lg shadow-md p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl font-bold text-red-600">{formatCurrency(getTotalSaldoKeluar())}</p>
            </div>
          ...
          <table className="min-w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Akun Bank</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredData.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.noTransaksi}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.akun}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getJenisTransaksiColor(item.jenisTransaksi)}`}>
                      {item.jenisTransaksi}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.tglTransaksi}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.referensi}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
