...
const ReturPenjualanForm = () => {
    ...
    const fetchInvoices = async () => {
        try {
            const response = await fetch('http://localhost:8000/api/return-sales/invoices');
            const data = await response.json();
            if (data.status === 'success') {
    ...
    const fetchInvoiceDetails = async (invoiceNumber) => {
        try {
            setLoading(true);
            const response = await fetch(`http://localhost:8000/api/return-sales/invoice-details/${invoiceNumber}`);
            const data = await response.json();
            if (data.status === 'success') {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            const response = await fetch('http://localhost:8000/api/return-sales', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="form-container">
            <PageHeader 
                title="Form Return Penjualan" 
                subtitle="Buat return penjualan untuk barang yang dikembalikan oleh customer"
            />
                                        ...
                                        {getFilteredInvoices().map((invoice) => (
                                            ...
                                            >
                                                ...
                                                <div className="item-detail">
                                                    Tanggal: {invoice.invoice_date} | Total: Rp {invoice.total.toLocaleString()}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                        ...
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <input
                                                    ...
                                                    onChange={(e) => handleItemChange(index, 'qty_return', parseInt(e.target.value) || 0)}
                                                    min="0"
                                                    max={item.qty_sold}
                                                    className="qty-input"
                                                />
