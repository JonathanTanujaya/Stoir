...
const ReturPembelianForm = () => {
    ...
    const fetchPurchaseDetails = async (purchaseId) => {
        try {
            const response = await fetch(`http://localhost:8000/api/return-purchases/purchase-details/${purchaseId}`);
            const data = await response.json();
            if (data.status === 'success') {
    ...
    const filteredSuppliers = suppliers.filter(supplier =>
        ...
        supplier.kodesupplier.toLowerCase().includes(supplierSearch.toLowerCase())
    );

    const filteredPurchases = purchases.filter(purchase =>
        purchase.display_text.toLowerCase().includes(purchaseSearch.toLowerCase())
    );
    ...
    return (
        <div className="container">
            <PageHeader 
                ...
                subtitle="Buat retur pembelian baru"
            />
                                            ...
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    <input
                                                        ...
                                                        onChange={(e) => handleItemChange(index, 'qty_return', parseFloat(e.target.value) || 0)}
                                                        className="qty-input"
                                                    />
                                                </td>
                                                <td>
