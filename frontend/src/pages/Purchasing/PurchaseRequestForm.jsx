...
import { FormErrors, validateRequired, validatePositiveNumber } from '../../components/FormValidation';
...
const PurchaseRequestForm = () => {
  ...
  const onSubmit = async (data) => {
    setSubmitting(true);
    try {
      const requestAmount = currentItems.reduce((sum, item) => sum + (item.quantity * item.estimatedPrice), 0);
            ...
            <CardContent>
              ...
              {budgetStatus[budgetCategory] && (
                <Box>
                  ...
                  <LinearProgress
                    variant="determinate"
                    value={(budgetStatus[budgetCategory].used / budgetStatus[budgetCategory].allocated) * 100}
                    sx={{ mt: 2, mb: 1 }}
                  />
                  
                  <Typography variant="caption" color="text.secondary">
                    {((budgetStatus[budgetCategory].used / budgetStatus[budgetCategory].allocated) * 100).toFixed(1)}% utilized
                  </Typography>
                </Box>
              )}
            </CardContent>
                  ...
                  {selectedProduct && (
                    <Box sx={{ p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
                      <Typography variant="caption">
                        ...
                        Est. Price: {formatCurrency(selectedProduct.modal)} | 
                        Total: {formatCurrency(selectedProduct.modal * productQuantity)}
                      </Typography>
                    </Box>
                  )}
                </Grid>
              ...
              {currentItems.length > 0 && (
                <Box sx={{ mt: 3, p: 2, bgcolor: 'primary.main', color: 'white', borderRadius: 1 }}>
                  <Typography variant="h6">
                    Total Request Value: {formatCurrency(
                      currentItems.reduce((sum, item) => sum + (item.quantity * item.estimatedPrice), 0)
                    )}
                  </Typography>
                  <Typography variant="body2">
                        ...
                        <TableRow key={analysis.supplier.id}>
                          ...
                          <TableCell align="center">
                            <Chip
                              ...
                              color={analysis.deliveryTime <= 3 ? 'success' : 
                                     analysis.deliveryTime <= 7 ? 'warning' : 'error'}
                              icon={<LocalShipping />}
                            />
                          </TableCell>
                          <TableCell align="center">
                            <Chip
                              ...
                              color={analysis.riskLevel === 'low' ? 'success' :
                          ...
                          <TableCell align="center">
                            <Chip
                              ...
                              color={analysis.recommendation === 'preferred' ? 'success' :
                                     analysis.recommendation === 'acceptable' ? 'info' : 'warning'}
                              icon={analysis.recommendation === 'preferred' ? <Star /> : undefined}
                            />
                          </TableCell>
                          <TableCell align="center">
                            <Button
                              variant={watchedValues.selectedSuppliers?.includes(analysis.supplier.id) ? 'contained' : 'outlined'}
                              size="small"
                              onClick={() => {
                                ...
                                if (isSelected) {
                                  setValue('selectedSuppliers', current.filter(id => id !== analysis.supplier.id));
                                } else {
                                  setValue('selectedSuppliers', [...current, analysis.supplier.id]);
                                }
                              }}
                            >
                              {watchedValues.selectedSuppliers?.includes(analysis.supplier.id) ? 'Selected' : 'Select'}
                            </Button>
                          </TableCell>
                        </TableRow>
                      ))}
  ...
  function renderPriceAnalysis() {
    return (
      <Typography variant="h6">Price Analysis Content - To be implemented</Typography>
    );
  }

  function renderApprovalWorkflow() {
    return (
      <Typography variant="h6">Approval Workflow Content - To be implemented</Typography>
    );
  }

  function renderPurchaseOrder() {
    return (
      <Typography variant="h6">Purchase Order Content - To be implemented</Typography>
    );
  }

  function renderConfirmation() {
    return (
      <Typography variant="h6">Confirmation Content - To be implemented</Typography>
    );
  }
