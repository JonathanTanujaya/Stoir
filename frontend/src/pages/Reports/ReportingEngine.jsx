...
const DATE_PRESETS = [
  ...
  { id: 'yesterday', name: 'Yesterday', getValue: () => ({ start: subDays(new Date(), 1), end: subDays(new Date(), 1) }) },
  { id: 'last7days', name: 'Last 7 Days', getValue: () => ({ start: subDays(new Date(), 7), end: new Date() }) },
  { id: 'last30days', name: 'Last 30 Days', getValue: () => ({ start: subDays(new Date(), 30), end: new Date() }) },
  { id: 'thismonth', name: 'This Month', getValue: () => ({ start: startOfMonth(new Date()), end: endOfMonth(new Date()) }) },
  { id: 'lastmonth', name: 'Last Month', getValue: () => ({ start: startOfMonth(subMonths(new Date(), 1)), end: endOfMonth(subMonths(new Date(), 1)) }) },
  { id: 'thisyear', name: 'This Year', getValue: () => ({ start: startOfYear(new Date()), end: endOfYear(new Date()) }) },
  { id: 'lastyear', name: 'Last Year', getValue: () => ({ start: startOfYear(subYears(new Date(), 1)), end: endOfYear(subYears(new Date(), 1)) }) },
...
const ReportingEngine = () => {
  ...
  // Report Data Query
  const { data: reportData, isLoading: isLoadingData, error: dataError, refetch } = useQuery({
    queryKey: ['reportData', selectedReport, reportConfig],
    queryFn: async () => {
      if (!selectedReport) return null;
      const endpoint = REPORT_TYPES
        .flatMap(type => type.reports)
        .find(report => report.id === selectedReport)?.endpoint;
    ...
    if (reportConfig.dateRange !== 'all' && data.length > 0) {
      ...
      if (dateRange) {
        data = data.filter(item => {
          const itemDate = new Date(item.tanggal || item.tglfaktur || item.tglpenerimaan || item.created_at);
      ...
      {selectedReportType && (
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 2, height: 'fit-content' }}>
            ...
            <List>
              {REPORT_TYPES
                .find(type => type.id === selectedReportType)
                ?.reports.map((report) => (
                  <ListItem
                    key={report.id}
                    button
                  ...
                  <Grid item xs={3}>
                    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={idLocale}>
                      <DatePicker
                        ...
                        onChange={(date) => setReportConfig(prev => ({ ...prev, customDateStart: date }))}
                  ...
                  <Grid item xs={3}>
                    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={idLocale}>
                      <DatePicker
                        ...
                        onChange={(date) => setReportConfig(prev => ({ ...prev, customDateEnd: date }))}
                        renderInput={(params) => <TextField {...params} />}
                      />
                    </LocalizationProvider>
                  </Grid>
                ...
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={(e) => setReportConfig(prev => ({ ...prev, isRealTime: e.target.checked }))}
                    />
                  }
              ...
              {reportConfig.isRealTime && (
                <Grid item xs={6}>
                  <TextField
                    ...
                    onChange={(e) => setReportConfig(prev => ({ ...prev, refreshInterval: parseInt(e.target.value) || 30 }))}
        ...
        <Grid container spacing={3}>
          {reportConfig.visualizations.map((vizType, index) => (
            <Grid item xs={12} md={vizType === 'table' ? 12 : 6} key={index}>
              <Paper sx={{ p: 2 }}>
                {renderVisualization(vizType, processedData)}
              </Paper>
            </Grid>
          ))}
        </Grid>
  ...
  const renderVisualization = (type, data) => {
    ...
    const textFields = Object.keys(firstItem).filter(key => 
      isNaN(parseFloat(firstItem[key])) || !isFinite(firstItem[key])
    );
    ...
    return (
      <Box>
        ...
        {data.length > maxRows ? (
          <List
            height={400}
            itemCount={Math.min(data.length, maxRows)}
            itemSize={40}
          >
            {TableRow}
          </List>
