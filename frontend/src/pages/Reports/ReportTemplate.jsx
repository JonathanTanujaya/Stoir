...
import { LineChart, BarChart, ResponsiveContainer, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, Line, Bar } from 'recharts';
...
const ReportTemplate = forwardRef(({
  ...
  const exportToPDF = async () => {
    ...
    if (lastUpdated) {
      yPos += 10;
      pdf.text(`Last Updated: ${format(new Date(lastUpdated), 'PPpp', { locale: idLocale })}`, 20, yPos);
    }
  ...
  const renderToolbar = () => (
    <Toolbar sx={{ bgcolor: 'background.paper', borderBottom: 1, borderColor: 'divider' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        {lastUpdated && (
          <Tooltip title={`Last updated: ${format(new Date(lastUpdated), 'PPpp', { locale: idLocale })}`}>
            <Chip
              icon={<Schedule />}
