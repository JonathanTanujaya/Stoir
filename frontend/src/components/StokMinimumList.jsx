...
const StokMinimumList = ({ onEdit, onRefresh }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/stok-minimum'); 
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus item ini?')) {
      try {
        // Ganti dengan endpoint API yang sesuai
        const response = await fetch(`/api/stok-minimum/${id}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        ...
        <TableBody>
          {data.map((row) => (
            ...
            >
              ...
              <TableCell>{row.stok_minimum}</TableCell>
              <TableCell>
...
export default StokMinimumList;
