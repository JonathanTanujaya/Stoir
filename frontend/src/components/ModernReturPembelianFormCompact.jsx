...
import { Plus, Search, Package, Calendar, FileText, Building, RotateCcw, Trash2 } from 'lucide-react';
...
const ModernReturPembelianFormCompact = () => {
  const [formData, setFormData] = useState({
    ...
    catatan: '',
    items: [
      { id: 1, kode: 'BRG001', nama: 'Sparepart Engine A100', qtyAsli: 5, qtyRetur: 2, satuan: 'PCS', harga: 150000, alasan: 'Rusak', subtotal: 300000 }
    ]
  });
  ...
  const calculateTotals = () => {
    const totalQtyRetur = formData.items.reduce((sum, item) => sum + item.qtyRetur, 0);
    const totalNilaiRetur = formData.items.reduce((sum, item) => sum + item.subtotal, 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4 border-l-4 border-red-500">
          <div className="flex items-center mb-4">
            <RotateCcw className="w-5 h-5 text-red-600 mr-2" />
            <h2 className="text-lg font-semibold text-gray-900">Form Retur Pembelian</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">NO FAKTUR ASLI</label>
              <div className="flex space-x-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-red-500 focus:border-transparent"
                />
                <button 
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Qty Asli</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {formData.items.map((item, index) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2 text-sm text-gray-900 text-right">{item.qtyAsli}</td>
                      <td className="px-3 py-2 text-sm text-red-600 text-right font-medium">{item.qtyRetur}</td>
                      ...
                      <td className="px-3 py-2 text-sm text-red-600 text-right font-medium">{formatCurrency(item.subtotal)}</td>
            ...
            <div className="bg-white rounded-lg shadow-sm border p-4 sticky top-4">
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                <div className="flex items-center">
                  ...
                  <span className="text-sm font-medium text-red-700">Transaksi Retur</span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-red-600">{totals.totalQtyRetur} pcs</span>
                </div>
                <div className="border-t pt-3">
                  <div className="flex justify-between">
                    <span className="font-semibold text-gray-900">Total Nilai Retur:</span>
                    <span className="font-bold text-red-600">{formatCurrency(totals.totalNilaiRetur)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="text-sm">
                  <div className="font-medium text-yellow-800">Status Persetujuan</div>
                  <div className="text-yellow-600 mt-1">Menunggu persetujuan supervisor</div>
                </div>
              </div>
