...
const MergeBarangList = ({ onEdit, onRefresh }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/merge-barang'); 
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus item ini?')) {
      try {
        // Ganti dengan endpoint API yang sesuai
        const response = await fetch(`/api/merge-barang/${id}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
            ...
            <TableCell>Barang Asal</TableCell>
            <TableCell>Barang Tujuan</TableCell>
        ...
        <TableBody>
          {data.map((row) => (
            ...
            >
              ...
              <TableCell>{row.barang_asal}</TableCell>
              <TableCell>{row.barang_tujuan}</TableCell>
              <TableCell>
...
export default MergeBarangList;
