...
const UserModuleList = ({ onEdit, onRefresh }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/user-module'); 
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus item ini?')) {
      try {
        // Ganti dengan endpoint API yang sesuai
        const response = await fetch(`/api/user-module/${id}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
            ...
            <TableCell>Nama Modul</TableCell>
        ...
        <TableBody>
          {data.map((row) => (
            ...
            >
              ...
              <TableCell>{row.nama_modul}</TableCell>
