...
export const purchasingAPI = {
  ...
  exportReport: (type, filters) => 
    apiClient.get(`/purchasing/export/${type}`, { params: filters }),
};
...
export const suppliersAPI = {
  ...
  updateCategory: (id, category) => 
    apiClient.put(`/suppliers/${id}/category`, { category }),
  ...
  // Compare suppliers
  compare: (supplierIds) => 
    apiClient.post('/suppliers/compare', { supplier_ids: supplierIds }),
  
  // Get market analysis
  getMarketAnalysis: (productId) => 
    apiClient.get(`/suppliers/market-analysis/${productId}`),
};
...
export const performanceAPI = {
  ...
  // Get KPI data
  getKPIs: (timeframe = '6months') =>
    apiClient.get('/analytics/kpis', { params: { timeframe } }),
...
export const analyticsAPI = {
  ...
  getPredictiveAnalysis: (type, data) =>
    apiClient.post(`/analytics/predictive/${type}`, data),
...
export const budgetAPI = {
  ...
  checkAvailability: (data) =>
    apiClient.post('/budget/check-availability', data),
  
  // Reserve budget
  reserveBudget: (data) =>
    apiClient.post('/budget/reserve', data),
  
  // Release budget
  releaseBudget: (reservationId) =>
    apiClient.delete(`/budget/reserve/${reservationId}`),
  
  // Get budget utilization
  getUtilization: (filters = {}) =>
    apiClient.get('/budget/utilization', { params: filters }),
};
...
export const offlineUtils = {
  ...
  isCacheable: (url, method) => {
    ...
    return method === 'GET' && 
           cacheableEndpoints.some(endpoint => url.includes(endpoint));
  },
